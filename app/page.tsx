...
export default function PaperX() {
  ...
  const SITE_URL = "https://paperx-b9wcoy74a-dangnguyenhai8696-1052s-projects.vercel.app"
  ...
  useEffect(() => {
    setCandleData(genCandles(60, activeToken.price, !activeToken.change.startsWith("-")))
  }, [activeToken])
  ...
  const chartSvg = () => {
    ...
    drawMA(ma7, "#EF9F27"); drawMA(ma25, "#378ADD")
    return out
  }
  ...
  const s = (label: string, value: string, good: boolean) => (
    <div key={label} style={{ display: "flex", justifyContent: "space-between", fontSize: 10 }}>
      <span style={{ color: "#5a5f78" }}>{label}</span>
      <span style={{ color: value === "N/A" ? "#5a5f78" : good ? UP : DN }}>{value}</span>
    </div>
  )
        ...
        <div style={{ padding: "12px 14px", background: "#0d0e12" }}>
          ...
          <div style={{ background: verdict(activeToken)==="SAFE"?"#0a1f12":verdict(activeToken)==="RISKY"?"#1f1508":"#1f0a0a", border:`1px solid ${vColor(verdict(activeToken))}40`, borderRadius:8, padding:"8px 10px", marginBottom:10 }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:3 }}>
              {s("Mint", activeToken.mint, activeToken.mint==="Revoked")}
              {s("LP", activeToken.lp, activeToken.lp.startsWith("Yes"))}
              {s("Top10", activeToken.top10, activeToken.top10==="N/A"||parseInt(activeToken.top10)<30)}
              {s("Honeypot", activeToken.honey, activeToken.honey==="No")}
            </div>
          </div>
      ...
      {tab === "referral" && (
        <div>
          ...
          <div style={{ margin:14, background:"#13151c", border:"1px solid #2a2d3a", borderRadius:12, padding:16 }}>
            ...
            <div style={{ fontSize:20, fontWeight:500, color:"#fff", letterSpacing:2, textAlign:"center", padding:12, background:"#0d0e12", borderRadius:8, marginBottom:6 }}>
              {refCode}
            </div>
